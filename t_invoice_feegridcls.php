...
$t_invoice_fee_grid = NULL; // Initialize page object first

class ct_invoice_fee_grid extends ct_invoice_fee {
	...
	// Page object name
	var $PageObjName = 't_invoice_fee_grid';

	// Grid form hidden field names
	var $FormName = 'ft_invoice_feegrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_invoice_fee"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->invoice_detail_id->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_fee_id") && $objForm->HasValue("o_fee_id") && $this->fee_id->CurrentValue <> $this->fee_id->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->invoice_detail_id->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('invoice_detail_id');
	}
	...
	function LoadFormValues() {
		...
		$this->fee_id->setOldValue($objForm->GetValue("o_fee_id"));
		...
		if (!$this->invoice_detail_id->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->invoice_detail_id->CurrentValue = strval($arKeys[0]); // invoice_detail_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "t_invoice") {
			...
			if ($GLOBALS["t_invoice"]->EventCancelled) $this->EventCancelled = TRUE;
		}
