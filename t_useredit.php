...
$t_user_edit = NULL; // Initialize page object first

class ct_user_edit extends ct_user {
	...
	function Page_Init() {
		...
		if (!$Security->CanEdit()) {
			...
			if ($Security->CanList())
				$this->Page_Terminate(ew_GetUrl("t_userlist.php"));
			else
	...
	function Page_Main() {
		...
		if ($this->user_id->CurrentValue == "") {
			$this->Page_Terminate("t_userlist.php"); // Invalid key, return to list
		}
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("t_userlist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				$sReturnUrl = $this->getReturnUrl();
				if (ew_GetPageName($sReturnUrl) == "t_userlist.php")
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, $this->AddMasterUrl("t_userlist.php"), "", $this->TableVar, TRUE);
		$PageId = "edit";
...
// Create page object
if (!isset($t_user_edit)) $t_user_edit = new ct_user_edit();

// Page init
$t_user_edit->Page_Init();

// Page main
$t_user_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_user_edit->Page_Render();
?>
...
var CurrentForm = ft_useredit = new ew_Form("ft_useredit", "edit");

// Validate form
ft_useredit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $t_user->userlevel->FldCaption(), $t_user->userlevel->ReqErrMsg)) ?>");
...
ft_useredit.Form_CustomValidate = 
...
ft_useredit.ValidateRequired = true;
<?php } else { ?>
ft_useredit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_useredit.Lists["x_userlevel"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ft_useredit.Lists["x_userlevel"].Options = <?php echo json_encode($t_user->userlevel->Options()) ?>;
...
<?php } ?>
<?php $t_user_edit->ShowPageHeader(); ?>
<?php
$t_user_edit->ShowMessage();
?>
<form name="ft_useredit" id="ft_useredit" class="<?php echo $t_user_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_user_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_user_edit->Token ?>">
<?php } ?>
...
<input type="text" data-table="t_user" data-field="x_username" name="x_username" id="x_username" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($t_user->username->getPlaceHolder()) ?>" value="<?php echo $t_user->username->EditValue ?>"<?php echo $t_user->username->EditAttributes() ?>>
</span>
...
<input type="text" data-password-strength="pst_password" data-password-generated="pgt_password" data-table="t_user" data-field="x_password" name="x_password" id="x_password" value="<?php echo $t_user->password->EditValue ?>" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($t_user->password->getPlaceHolder()) ?>"<?php echo $t_user->password->EditAttributes() ?>>
...
<?php if ($t_user->userlevel->Visible) { // userlevel ?>
	<div id="r_userlevel" class="form-group">
		<label id="elh_t_user_userlevel" for="x_userlevel" class="col-sm-2 control-label ewLabel"><?php echo $t_user->userlevel->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $t_user->userlevel->CellAttributes() ?>>
<?php if (!$Security->IsAdmin() && $Security->IsLoggedIn()) { // Non system admin ?>
<span id="el_t_user_userlevel">
<p class="form-control-static"><?php echo $t_user->userlevel->EditValue ?></p>
</span>
<?php } else { ?>
<span id="el_t_user_userlevel">
<select data-table="t_user" data-field="x_userlevel" data-value-separator="<?php echo $t_user->userlevel->DisplayValueSeparatorAttribute() ?>" id="x_userlevel" name="x_userlevel"<?php echo $t_user->userlevel->EditAttributes() ?>>
<?php echo $t_user->userlevel->SelectOptionListHtml("x_userlevel") ?>
</select>
</span>
<?php } ?>
<?php echo $t_user->userlevel->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_user_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
