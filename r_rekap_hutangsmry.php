...
<?php include_once "r_rekap_hutangsmryinfo.php" ?>
<?php
...
$r_rekap_hutang_summary = NULL; // Initialize page object first

class crr_rekap_hutang_summary extends crr_rekap_hutang {
	...
	// Page object name
	var $PageObjName = 'r_rekap_hutang_summary';
	...
	function __construct() {
		...
		// Table object (r_rekap_hutang)
		if (!isset($GLOBALS["r_rekap_hutang"])) {
			$GLOBALS["r_rekap_hutang"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["r_rekap_hutang"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'r_rekap_hutang', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fr_rekap_hutangsummary";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'r_rekap_hutang');
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_r_rekap_hutang\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_r_rekap_hutang',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fr_rekap_hutangsummary\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fr_rekap_hutangsummary\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fr_rekap_hutangsummary\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function GetExtendedFilter() {
		...
		// Reset search command
			...
			if ($this->GetDropDownValue($this->nama)) {
				$bSetupFilter = TRUE;
			} elseif ($this->nama->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_r_rekap_hutang_nama'])) {
				$bSetupFilter = TRUE;
			}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_r_rekap_hutang_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r_rekap_hutang_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_r_rekap_hutang_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r_rekap_hutang_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_r_rekap_hutang_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_r_rekap_hutang_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_r_rekap_hutang_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_r_rekap_hutang_' . $parm] = $sv;
		$_SESSION['so_r_rekap_hutang_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_r_rekap_hutang_' . $parm] = $sv1;
		$_SESSION['so_r_rekap_hutang_' . $parm] = $so1;
		$_SESSION['sc_r_rekap_hutang_' . $parm] = $sc;
		$_SESSION['sv2_r_rekap_hutang_' . $parm] = $sv2;
		$_SESSION['so2_r_rekap_hutang_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_r_rekap_hutang_$parm"] = "";
		$_SESSION["rf_r_rekap_hutang_$parm"] = "";
		$_SESSION["rt_r_rekap_hutang_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_r_rekap_hutang_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_r_rekap_hutang_$parm"];
		$fld->RangeTo = @$_SESSION["rt_r_rekap_hutang_$parm"];
	}
...
// Create page object
if (!isset($r_rekap_hutang_summary)) $r_rekap_hutang_summary = new crr_rekap_hutang_summary();
if (isset($Page)) $OldPage = $Page;
$Page = &$r_rekap_hutang_summary;
...
// Create page object
var r_rekap_hutang_summary = new ewr_Page("r_rekap_hutang_summary");

// Page properties
r_rekap_hutang_summary.PageID = "summary"; // Page ID
var EWR_PAGE_ID = r_rekap_hutang_summary.PageID;

// Extend page with Chart_Rendering function
r_rekap_hutang_summary.Chart_Rendering = 
...
r_rekap_hutang_summary.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var CurrentForm = fr_rekap_hutangsummary = new ewr_Form("fr_rekap_hutangsummary");

// Validate method
fr_rekap_hutangsummary.Validate = function() {
...
fr_rekap_hutangsummary.Form_CustomValidate = 
...
fr_rekap_hutangsummary.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fr_rekap_hutangsummary.ValidateRequired = false; // No JavaScript validation
<?php } ?>

// Use Ajax
fr_rekap_hutangsummary.Lists["sv_nama"] = {"LinkField":"sv_nama","Ajax":true,"DisplayFields":["sv_nama","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<script type="text/javascript">
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fr_rekap_hutangsummary" id="fr_rekap_hutangsummary" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fr_rekap_hutangsummary_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<select data-table="r_rekap_hutang" data-field="x_nama" data-value-separator="<?php echo ewr_HtmlEncode(is_array($Page->nama->DisplayValueSeparator) ? json_encode($Page->nama->DisplayValueSeparator) : $Page->nama->DisplayValueSeparator) ?>" id="sv_nama" name="sv_nama"<?php echo $Page->nama->EditAttributes() ?>>
...
<script type="text/javascript">
fr_rekap_hutangsummary.Init();
fr_rekap_hutangsummary.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php include "r_rekap_hutangsmrypager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php if ($Page->Export <> "pdf") { ?>
...
<?php } ?>
<span data-class="tpb<?php echo $Page->GrpCount-1 ?>_r_rekap_hutang"><?php echo $Page->PageBreakContent ?></span>
<?php } ?>
<?php if ($Page->Export <> "pdf") { ?>
...
<?php if ($Page->nama->Visible) { ?>
	...
	<?php } else { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="nama"><div class="r_rekap_hutang_nama"><span class="ewTableHeaderCaption"><?php echo $Page->nama->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="nama">
<?php if ($Page->SortUrl($Page->nama) == "") { ?>
		<div class="ewTableHeaderBtn r_rekap_hutang_nama">
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="tgl_pelaksanaan"><div class="r_rekap_hutang_tgl_pelaksanaan"><span class="ewTableHeaderCaption"><?php echo $Page->tgl_pelaksanaan->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="tgl_pelaksanaan">
<?php if ($Page->SortUrl($Page->tgl_pelaksanaan) == "") { ?>
		<div class="ewTableHeaderBtn r_rekap_hutang_tgl_pelaksanaan">
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="no_kwitansi"><div class="r_rekap_hutang_no_kwitansi"><span class="ewTableHeaderCaption"><?php echo $Page->no_kwitansi->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="no_kwitansi">
<?php if ($Page->SortUrl($Page->no_kwitansi) == "") { ?>
		<div class="ewTableHeaderBtn r_rekap_hutang_no_kwitansi">
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="nomor"><div class="r_rekap_hutang_nomor"><span class="ewTableHeaderCaption"><?php echo $Page->nomor->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="nomor">
<?php if ($Page->SortUrl($Page->nomor) == "") { ?>
		<div class="ewTableHeaderBtn r_rekap_hutang_nomor">
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="total_ppn"><div class="r_rekap_hutang_total_ppn" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->total_ppn->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="total_ppn">
<?php if ($Page->SortUrl($Page->total_ppn) == "") { ?>
		<div class="ewTableHeaderBtn r_rekap_hutang_total_ppn" style="text-align: right;">
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
		<span class="ewSummaryCaption r_rekap_hutang_nama"><span class="ewTableHeaderCaption"><?php echo $Page->nama->FldCaption() ?></span></span>
<?php } else { ?>
	<?php if ($Page->SortUrl($Page->nama) == "") { ?>
		<span class="ewSummaryCaption r_rekap_hutang_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->nama->FldCaption() ?></span>
		</span>
	<?php } else { ?>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_r_rekap_hutang_nama"<?php echo $Page->nama->ViewAttributes() ?>><?php echo $Page->nama->GroupViewValue ?></span>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_r_rekap_hutang_nama"<?php echo $Page->nama->ViewAttributes() ?>><?php echo $Page->nama->GroupViewValue ?></span></td>
	<?php } ?>
<?php } ?>
<?php if ($Page->tgl_pelaksanaan->Visible) { ?>
		<td data-field="tgl_pelaksanaan"<?php echo $Page->tgl_pelaksanaan->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_r_rekap_hutang_tgl_pelaksanaan"<?php echo $Page->tgl_pelaksanaan->ViewAttributes() ?>><?php echo $Page->tgl_pelaksanaan->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->no_kwitansi->Visible) { ?>
		<td data-field="no_kwitansi"<?php echo $Page->no_kwitansi->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_r_rekap_hutang_no_kwitansi"<?php echo $Page->no_kwitansi->ViewAttributes() ?>><?php echo $Page->no_kwitansi->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->nomor->Visible) { ?>
		<td data-field="nomor"<?php echo $Page->nomor->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_r_rekap_hutang_nomor"<?php echo $Page->nomor->ViewAttributes() ?>><?php echo $Page->nomor->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->total_ppn->Visible) { ?>
		<td data-field="total_ppn"<?php echo $Page->total_ppn->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_r_rekap_hutang_total_ppn"<?php echo $Page->total_ppn->ViewAttributes() ?>><?php echo $Page->total_ppn->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<?php if ($Page->total_ppn->Visible) { ?>
		<td data-field="total_ppn"<?php echo $Page->total_ppn->CellAttributes() ?>>
<span data-class="tpgs<?php echo $Page->GrpCount ?>_r_rekap_hutang_total_ppn"<?php echo $Page->total_ppn->ViewAttributes() ?>><?php echo $Page->total_ppn->SumViewValue ?></span></td>
<?php } ?>
	</tr>
<?php } ?>
<?php
...
<?php } ?>
<?php if ($Page->total_ppn->Visible) { ?>
		<td data-field="total_ppn"<?php echo $Page->total_ppn->CellAttributes() ?>><?php echo $ReportLanguage->Phrase("RptSum") ?>=<span data-class="tpps_r_rekap_hutang_total_ppn"<?php echo $Page->total_ppn->ViewAttributes() ?>><?php echo $Page->total_ppn->SumViewValue ?></span></td>
<?php } ?>
	</tr>
<?php } else { ?>
...
<?php if ($Page->total_ppn->Visible) { ?>
		<td data-field="total_ppn"<?php echo $Page->total_ppn->CellAttributes() ?>>
<span data-class="tpps_r_rekap_hutang_total_ppn"<?php echo $Page->total_ppn->ViewAttributes() ?>><?php echo $Page->total_ppn->SumViewValue ?></span></td>
<?php } ?>
	</tr>
<?php } ?>
<?php } ?>
...
<?php } ?>
<?php if ($Page->total_ppn->Visible) { ?>
		<td data-field="total_ppn"<?php echo $Page->total_ppn->CellAttributes() ?>><?php echo $ReportLanguage->Phrase("RptSum") ?>=<span data-class="tpts_r_rekap_hutang_total_ppn"<?php echo $Page->total_ppn->ViewAttributes() ?>><?php echo $Page->total_ppn->SumViewValue ?></span></td>
<?php } ?>
	</tr>
<?php } else { ?>
...
<?php if ($Page->total_ppn->Visible) { ?>
		<td data-field="total_ppn"<?php echo $Page->total_ppn->CellAttributes() ?>>
<span data-class="tpts_r_rekap_hutang_total_ppn"<?php echo $Page->total_ppn->ViewAttributes() ?>><?php echo $Page->total_ppn->SumViewValue ?></span></td>
<?php } ?>
	</tr>
<?php } ?>
	</tfoot>
