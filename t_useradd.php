...
$t_user_add = NULL; // Initialize page object first

class ct_user_add extends ct_user {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "t_userview.php")
...
// Create page object
if (!isset($t_user_add)) $t_user_add = new ct_user_add();

// Page init
$t_user_add->Page_Init();

// Page main
$t_user_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_user_add->Page_Render();
?>
...
var CurrentForm = ft_useradd = new ew_Form("ft_useradd", "add");

// Validate form
ft_useradd.Validate = function() {
...
ft_useradd.Form_CustomValidate = 
...
ft_useradd.ValidateRequired = true;
<?php } else { ?>
ft_useradd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_useradd.Lists["x_userlevel"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ft_useradd.Lists["x_userlevel"].Options = <?php echo json_encode($t_user->userlevel->Options()) ?>;
...
<?php } ?>
<?php $t_user_add->ShowPageHeader(); ?>
<?php
$t_user_add->ShowMessage();
?>
<form name="ft_useradd" id="ft_useradd" class="<?php echo $t_user_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_user_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_user_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_user_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
