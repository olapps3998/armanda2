...
$t_invoice_fee_view = NULL; // Initialize page object first

class ct_invoice_fee_view extends ct_invoice_fee {
	...
	// Page object name
	var $PageObjName = 't_invoice_fee_view';
	...
	function __construct() {
		...
		if (@$_GET["invoice_detail_id"] <> "") {
			$this->RecKey["invoice_detail_id"] = $_GET["invoice_detail_id"];
			$KeyUrl .= "&amp;invoice_detail_id=" . urlencode($this->RecKey["invoice_detail_id"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["invoice_detail_id"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["invoice_detail_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["invoice_detail_id"] <> "") {
				...
				$this->RecKey["invoice_detail_id"] = $this->invoice_detail_id->QueryStringValue;
			} elseif (@$_POST["invoice_detail_id"] <> "") {
				$this->invoice_detail_id->setFormValue($_POST["invoice_detail_id"]);
				$this->RecKey["invoice_detail_id"] = $this->invoice_detail_id->FormValue;
			} else {
				$sReturnUrl = "t_invoice_feelist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "t_invoice_feelist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "t_invoice_feelist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($t_invoice_fee_view)) $t_invoice_fee_view = new ct_invoice_fee_view();

// Page init
$t_invoice_fee_view->Page_Init();

// Page main
$t_invoice_fee_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_invoice_fee_view->Page_Render();
?>
...
var CurrentForm = ft_invoice_feeview = new ew_Form("ft_invoice_feeview", "view");

// Form_CustomValidate event
ft_invoice_feeview.Form_CustomValidate = 
...
ft_invoice_feeview.ValidateRequired = true;
<?php } else { ?>
ft_invoice_feeview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_invoice_feeview.Lists["x_fee_id"] = {"LinkField":"x_fee_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_jenis","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t_fee"};
...
<?php if ($t_invoice_fee->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$t_invoice_fee_view->IsModal) { ?>
<?php if ($t_invoice_fee->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $t_invoice_fee_view->ExportOptions->Render("body") ?>
<?php
	foreach ($t_invoice_fee_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$t_invoice_fee_view->IsModal) { ?>
<?php if ($t_invoice_fee->Export == "") { ?>
...
<?php } ?>
<?php $t_invoice_fee_view->ShowPageHeader(); ?>
<?php
$t_invoice_fee_view->ShowMessage();
?>
<form name="ft_invoice_feeview" id="ft_invoice_feeview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_invoice_fee_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_invoice_fee_view->Token ?>">
<?php } ?>
...
<?php if ($t_invoice_fee->fee_id->Visible) { // fee_id ?>
	<tr id="r_fee_id">
		<td><span id="elh_t_invoice_fee_fee_id"><?php echo $t_invoice_fee->fee_id->FldCaption() ?></span></td>
...
<?php echo $t_invoice_fee->fee_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($t_invoice_fee->harga->Visible) { // harga ?>
	<tr id="r_harga">
		<td><span id="elh_t_invoice_fee_harga"><?php echo $t_invoice_fee->harga->FldCaption() ?></span></td>
...
<?php echo $t_invoice_fee->harga->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($t_invoice_fee->qty->Visible) { // qty ?>
	<tr id="r_qty">
		<td><span id="elh_t_invoice_fee_qty"><?php echo $t_invoice_fee->qty->FldCaption() ?></span></td>
...
<?php echo $t_invoice_fee->qty->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($t_invoice_fee->satuan->Visible) { // satuan ?>
	<tr id="r_satuan">
		<td><span id="elh_t_invoice_fee_satuan"><?php echo $t_invoice_fee->satuan->FldCaption() ?></span></td>
...
<?php echo $t_invoice_fee->satuan->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($t_invoice_fee->jumlah->Visible) { // jumlah ?>
	<tr id="r_jumlah">
		<td><span id="elh_t_invoice_fee_jumlah"><?php echo $t_invoice_fee->jumlah->FldCaption() ?></span></td>
		<td data-name="jumlah"<?php echo $t_invoice_fee->jumlah->CellAttributes() ?>>
<span id="el_t_invoice_fee_jumlah">
<span<?php echo $t_invoice_fee->jumlah->ViewAttributes() ?>>
<?php echo $t_invoice_fee->jumlah->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($t_invoice_fee->keterangan->Visible) { // keterangan ?>
	<tr id="r_keterangan">
		<td><span id="elh_t_invoice_fee_keterangan"><?php echo $t_invoice_fee->keterangan->FldCaption() ?></span></td>
...
<?php echo $t_invoice_fee->keterangan->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($t_invoice_fee->Export == "") { ?>
<script type="text/javascript">
ft_invoice_feeview.Init();
</script>
<?php } ?>
<?php
$t_invoice_fee_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($t_invoice_fee->Export == "") { ?>
...
<?php
$t_invoice_fee_view->Page_Terminate();
?>
