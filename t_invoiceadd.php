...
$t_invoice_add = NULL; // Initialize page object first

class ct_invoice_add extends ct_invoice {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "t_invoiceview.php")
	...
	function LoadDefaultValues() {
		...
		$this->pasal23->CurrentValue = NULL;
	...
	function ValidateForm() {
		...
		if (in_array("t_invoice_fee", $DetailTblVar) && $GLOBALS["t_invoice_fee"]->DetailAdd) {
		...
		if (in_array("t_invoice_pelaksanaan", $DetailTblVar) && $GLOBALS["t_invoice_pelaksanaan"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("t_invoice_fee", $DetailTblVar) && $GLOBALS["t_invoice_fee"]->DetailAdd) {
				$GLOBALS["t_invoice_fee"]->invoice_id->setSessionValue($this->invoice_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["t_invoice_fee_grid"])) $GLOBALS["t_invoice_fee_grid"] = new ct_invoice_fee_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "t_invoice_fee"); // Load user level of detail table
				$AddRow = $GLOBALS["t_invoice_fee_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["t_invoice_fee"]->invoice_id->setSessionValue(""); // Clear master key if insert failed
			}
			if (in_array("t_invoice_pelaksanaan", $DetailTblVar) && $GLOBALS["t_invoice_pelaksanaan"]->DetailAdd) {
				$GLOBALS["t_invoice_pelaksanaan"]->invoice_id->setSessionValue($this->invoice_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["t_invoice_pelaksanaan_grid"])) $GLOBALS["t_invoice_pelaksanaan_grid"] = new ct_invoice_pelaksanaan_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "t_invoice_pelaksanaan"); // Load user level of detail table
				$AddRow = $GLOBALS["t_invoice_pelaksanaan_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["t_invoice_pelaksanaan"]->invoice_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("t_invoice_fee", $DetailTblVar)) {
				...
				if ($GLOBALS["t_invoice_fee_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["t_invoice_fee_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["t_invoice_fee_grid"]->CurrentMode = "add";
					$GLOBALS["t_invoice_fee_grid"]->CurrentAction = "gridadd";
			...
			if (in_array("t_invoice_pelaksanaan", $DetailTblVar)) {
				...
				if ($GLOBALS["t_invoice_pelaksanaan_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["t_invoice_pelaksanaan_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["t_invoice_pelaksanaan_grid"]->CurrentMode = "add";
					$GLOBALS["t_invoice_pelaksanaan_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($t_invoice_add)) $t_invoice_add = new ct_invoice_add();

// Page init
$t_invoice_add->Page_Init();

// Page main
$t_invoice_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_invoice_add->Page_Render();
?>
...
var CurrentForm = ft_invoiceadd = new ew_Form("ft_invoiceadd", "add");

// Validate form
ft_invoiceadd.Validate = function() {
...
ft_invoiceadd.Form_CustomValidate = 
...
ft_invoiceadd.ValidateRequired = true;
<?php } else { ?>
ft_invoiceadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_invoiceadd.Lists["x_customer_id"] = {"LinkField":"x_customer_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t_customer"};
ft_invoiceadd.Lists["x_terbayar"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ft_invoiceadd.Lists["x_terbayar"].Options = <?php echo json_encode($t_invoice->terbayar->Options()) ?>;
ft_invoiceadd.Lists["x_pasal23"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ft_invoiceadd.Lists["x_pasal23"].Options = <?php echo json_encode($t_invoice->pasal23->Options()) ?>;
...
<?php } ?>
<?php $t_invoice_add->ShowPageHeader(); ?>
<?php
$t_invoice_add->ShowMessage();
?>
<form name="ft_invoiceadd" id="ft_invoiceadd" class="<?php echo $t_invoice_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_invoice_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_invoice_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("ft_invoiceadd", "x_tanggal", 7);
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("ft_invoiceadd", "x_tgl_bayar", 7);
</script>
<?php } ?>
</span>
...
<?php
	$t_invoice_add->DetailPages->ValidKeys = explode(",", $t_invoice->getCurrentDetailTable());
	$FirstActiveDetailTable = $t_invoice_add->DetailPages->ActivePageIndex();
?>
<div class="ewDetailPages">
<div class="tabbable" id="t_invoice_add_details">
	<ul class="nav<?php echo $t_invoice_add->DetailPages->NavStyle() ?>">
<?php
	if (in_array("t_invoice_fee", explode(",", $t_invoice->getCurrentDetailTable())) && $t_invoice_fee->DetailAdd) {
		if ($FirstActiveDetailTable == "" || $FirstActiveDetailTable == "t_invoice_fee") {
			$FirstActiveDetailTable = "t_invoice_fee";
		}
?>
		<li<?php echo $t_invoice_add->DetailPages->TabStyle("t_invoice_fee") ?>><a href="#tab_t_invoice_fee" data-toggle="tab"><?php echo $Language->TablePhrase("t_invoice_fee", "TblCaption") ?></a></li>
<?php
	}
?>
<?php
	if (in_array("t_invoice_pelaksanaan", explode(",", $t_invoice->getCurrentDetailTable())) && $t_invoice_pelaksanaan->DetailAdd) {
		if ($FirstActiveDetailTable == "" || $FirstActiveDetailTable == "t_invoice_pelaksanaan") {
			$FirstActiveDetailTable = "t_invoice_pelaksanaan";
		}
?>
		<li<?php echo $t_invoice_add->DetailPages->TabStyle("t_invoice_pelaksanaan") ?>><a href="#tab_t_invoice_pelaksanaan" data-toggle="tab"><?php echo $Language->TablePhrase("t_invoice_pelaksanaan", "TblCaption") ?></a></li>
<?php
	}
?>
	</ul>
...
?>
		<div class="tab-pane<?php echo $t_invoice_add->DetailPages->PageStyle("t_invoice_fee") ?>" id="tab_t_invoice_fee">
...
?>
		<div class="tab-pane<?php echo $t_invoice_add->DetailPages->PageStyle("t_invoice_pelaksanaan") ?>" id="tab_t_invoice_pelaksanaan">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_invoice_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
